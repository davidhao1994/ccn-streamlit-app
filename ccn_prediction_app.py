        ...
        with tab3:
            ...
            if 'df_with_modes' not in st.session_state:
                ...
                **Training Configuration:**
                ...
                # Auto-detect meteorological features (common column name variations)
                met_feature_patterns = ['temperature', 'temp', 'rh', 'humidity', 'wind', 'pressure', 'precip']
                met_features = [col for col in df_with_modes.columns 
                               if any(pattern in col.lower() for pattern in met_feature_patterns)]
                
                # Auto-detect chemical features (common column name variations)
                chem_feature_patterns = ['so2', 'nox', 'no2', 'o3', 'ozone', 'organic', 'sulfate', 'sulphate', 
                                        'nh3', 'nh4', 'ammonium', 'nitrate', 'bc', 'pm']
                chem_features = [col for col in df_with_modes.columns 
                                if any(pattern in col.lower() for pattern in chem_feature_patterns)]
                ...
                **Available Features:**
                ...
                - Meteorological: {len(available_met)} features {f'({", ".join(available_met[:5])}, ...)' if len(available_met) > 5 else f'({", ".join(available_met)})' if available_met else '(none found)'}
                - Chemical: {len(available_chem)} features {f'({", ".join(available_chem[:5])}, ...)' if len(available_chem) > 5 else f'({", ".join(available_chem)})' if available_chem else '(none found)'}
                """)
