            ...
            **Purpose**: Analyze which particle size bins are most important for predicting CCN concentration 
            ...
            if 'N_CCN' not in df.columns:
                ...
                with col1:
                    ...
                    # Set default min to 50 nm or min_available if min_available > 50
                    default_min_diameter = max(50.0, float(min_available))
                    # Set default max to max_available
                    default_max_diameter = float(max_available)
                    
                    analysis_diameter_range = st.slider(
                        "Diameter range for analysis (nm)",
                        min_value=float(min_available),
                        max_value=float(max_available),
                        value=(default_min_diameter, default_max_diameter),
                        step=1.0,
                        help="CCN typically activate from particles > 50 nm"
                    )
                    
                    # Minimum diameter threshold
                    # Set default critical diameter to 66.5 nm, but ensure it's within valid range
                    default_critical_diameter = min(max(66.5, float(min_available)), float(max_available))
                    
                    critical_diameter = st.number_input(
                        "Critical diameter threshold (nm)",
                        min_value=float(min_available),
                        max_value=float(max_available),
                        value=default_critical_diameter,
                        step=0.5,
